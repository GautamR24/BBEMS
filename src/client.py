...
def newRegistration():
    ...
    sk = SigningKey.generate(curve=NIST521p) #(sign) encrypts usign private key ,(verify) decrypts using public 
    vk = sk.verifying_key
    ...
    if(arg_num < 3):
        ...
        if(arg1 == '-n' and arg2 == '-r'):
            ...
            if success:
                ...
                metadata = uploadFile(encryption_object)#metadata
            ...
            if success:
                ...
                message_received = sendDataToServer(message_to_send)#return the respomse from server
                if message_received[0] == 'D1D0' and message_received[1] and message_received[2][0]:#key and metadata
                    seed = message_received[1]
                    aes_object = AESCipher(seed)#key
                    address = aes_object.decrypt(message_received[2][0])#address                                                                
